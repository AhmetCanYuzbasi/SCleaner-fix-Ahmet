...
public class Grenade : MonoBehaviour, IEquipment
{
    ...
    SuctionAbility _suctionAbility;
    Explode _explosionAbility;
    ...
    void Awake(){
        ...
        _suctionAbility = GetComponent<SuctionAbility>();
        _explosionAbility = GetComponent<Explode>();
    }
    ...
    void Reset(){
        ...
        if(OnAbilityEnd != null){
            print(OnAbilityEnd.GetInvocationList().Length);
        }
        else{
            print("not null");
        }
        ...
        //_suctionAbility.enabled = false;
        enabled = false;
        gameObject.SetActive(false);
    }

    void TriggerExplosion(List<IDamageable> enemies){
        _explosionAbility.StartExplosion(enemies);
    }
